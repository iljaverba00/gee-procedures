  ...
  <q-card-section class="display:inline" v-if="procedureName && procedureGroup">
    <div class="text-subtitle1">
      ...
      <q-checkbox class="float-right" v-model="saveSet" left-label label="Сохранение"
        ><q-tooltip>Сохранение введённых данных</q-tooltip></q-checkbox
      >
    </div>
...
import Params from 'components/procedure/pages/Params.vue';
import { ref } from 'vue';
...
export default defineComponent({
  ...
  methods: {
    saveProcData(params) {
      ...
      if (!this.saveSet) {
        let ind = saveProcData.findIndex((obj) => obj.id == this.procedureId);
      ...
      let index = saveProcData.findIndex((obj) => obj.id == this.procedureId);
    ...
    checkSave() {
      ...
      const saveProcData = JSON.parse(data);
      const obj = saveProcData.find((el) => el.id == this.procedureId);
